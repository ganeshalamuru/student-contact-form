...
const VALID_SORT_FIELDS = ['name', 'fathername', 'phone', 'district', 'collegename', 'State', 'group', 'status'];

// Server-side paginated query using $facet.
// At least one of district / college / state must be present — enforced by the API layer.
export async function queryStudents({ page = 1, limit = 25, search = '', district = '', college = '', state = '', status = '', sort = '', order = 'asc' } = {}) {
  ...
  // 'New' must also match documents where the status field is missing
  if (status) {
    if (status === 'New') {
      match.$and = [{ $or: [{ status: 'New' }, { status: { $exists: false } }, { status: null }] }];
    } else {
      match.status = status;
    }
  }
  ...
  if (search?.trim()) {
    ...
    // Merge with existing $and (used by status='New' filter) to avoid $or key conflict
    if (match.$and) {
...
// Appends a new comment and transitions status New → Follow Up (if applicable).
export async function appendComment(id, text, createdBy = 'admin') {
  const col = await getCollection();

  const current = await col.findOne({ _id: new ObjectId(id) }, { projection: { status: 1 } });
  if (!current) throw new Error('Student not found');

  const currentStatus = current.status ?? 'New';
  const newStatus     = currentStatus === 'New' ? 'Follow Up' : currentStatus;

  const now     = new Date();
  const updated = await col.findOneAndUpdate(
    { _id: new ObjectId(id) },
    {
      $push: { comments: { text, createdAt: now, createdBy } },
      $set:  { updatedAt: now, status: newStatus },
    },
    { returnDocument: 'after' },
  );
  if (!updated) throw new Error('Student not found');
  return { comments: updated.comments ?? [], status: updated.status ?? 'New' };
}

// Updates status manually; only Interested and Closed are valid manual targets.
export async function updateStudentStatus(id, status) {
  if (!['Interested', 'Closed'].includes(status)) {
    throw new Error(`Manual status must be Interested or Closed, got: ${status}`);
  }
  const col     = await getCollection();
  const updated = await col.findOneAndUpdate(
    { _id: new ObjectId(id) },
